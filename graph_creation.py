...
class TitleDictionary:
    ...
    def _create_title_dict(self):
        ...
        title_dict = self.df.groupby('nconst')['primaryTitle'].apply(list).to_dict()
    ...
    def _create_profession_dict(self):
        ...
        def modify(row):
            return row['primaryName'] + "_" + row['primaryProfession'][0]
        self.df['newColumn'] = self.df.apply(lambda row: modify(row), axis=1)
        profession_dict = self.df.groupby('nconst')['newColumn'].first().to_dict()
...
class MovieNetwork:
    ...
    def add_edge(self, node1, node2, nconst_ar_dr, weight=1):
        ...
        self.add_node(node2)
        if(weight > 2):
            prof1 = nconst_ar_dr[node1].split("_")[1]
            prof2 = nconst_ar_dr[node2].split("_")[1]
            if prof1 == prof2:
                self.graph[node1][node2] = weight
                self.graph[node2][node1] = weight
            elif prof1 == 'd' and prof2 == 'a':
                self.graph[node1][node2] = weight
            elif prof2 == 'd' and prof1 == 'a':
                self.graph[node2][node1] = weight
        return
    ...
    def create_graph(self):
        ...
        #example graph looks like:
          ...
          #    'nm6225202': {}}
        nodeList = list(self.nconst_ar_dr.keys())
        for i in range(0,len(nodeList)):
            for j in range(i+1,len(nodeList)):
                node1 = nodeList[i]
                node2 = nodeList[j]
                weight = len(set(self.name_movie_dict[node1]).intersection(self.name_movie_dict[node2]))
